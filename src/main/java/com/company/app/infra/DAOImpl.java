...
/**
 *  Implements Bussined Logic(EJB) all  DAO should extend this class and call the method {@link #init(EntityManager)} in the
 *  @link PostConstruct} 
 * 
...
public abstract class DAOImpl<Entity extends BaseEntity<? extends Serializable>> implements DAO<Entity> {
  ...
  /**
   * This method needs to be call in method noted with PostConstruct in the DAO
   * @param em
   */
  ...
  public Entity find(AppQueryVO iseQuery) {
    ...
    if (log.isDebugEnabled()) log.debug("*** [Entity find(Entity ed, IseQueryVO<Entity> iseQuery)] " + (f-i) + " ms.");
  ...
  public List<Entity> list(AppQueryVO iseQuery) {
    ...
    if (iseQuery.isCount())
    	iseQuery = iseQuery.getBuilder().count(false).build();
    List<Entity> list =  BD.list(iseQuery);
    int j = 0; for (Entity edItem : list) {
      list.set(j++, hibernateProxy2Impl(edItem));
    }
    long f = System.currentTimeMillis();
    if (log.isDebugEnabled()) log.debug("*** [List<Entity> list(Entity ed, IseQueryVO<Entity> iseQuery)] executou em: " + (f-i) + " ms.");
    return list;
  }
  ...
  public long count(AppQueryVO iseQuery) {
    ...
    long f = System.currentTimeMillis();
    if (log.isDebugEnabled()) log.debug("*** [Number count(Entity ed, IseQueryVO<Entity> iseQuery)] executou em: " + (f-i) + " ms.");
