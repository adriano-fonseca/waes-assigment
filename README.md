...
a **docker-compose.yml**, that uses some images that I have build and pushed to docker hub to run this assignment.
...
* Provide 2 http end points that accepts JSON base64 encoded binary data on both endpoints o <host>/v1/diff/<ID>/left and <host>/v1/diff/<ID>/right
...
To Integration Tests I am using Arquillian, to run Unit and Integration tests run the command bellow and the Arquillian chameleon will download a Wildfly container and will configure
...
could establish a relationship among them, doing that we could have diff that combine data already persisted. For example:
...
Of course this is an improvement suggestion. Currently we are on the middle of the path, but we could implement that with little effort. 
...
This project are Ready to be a multistage pipeline and enable to produce the Java artefact and The docker image to this application, this brings huge 
...
stack and easily scale up and down.  

# Interacting with API

There are two ways to do data. Using the Swagger in  **http://<host>:8080/waes/apidocs** or Using some Http Client. I suggest to use Postman, you can find
a export from it with the end points and some initial data [here](https://github.com/adriano-fonseca/waes-assigment/blob/master/src/main/postman/diff_waes_postman.json) 
...
implement the end points to update left and right data. 
...
I know that documentation and test could be more extensive, however the time was a little tight to everything that I would like to do.
